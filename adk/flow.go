...
// SetSubAgents 为给定的 Agent 设置子 Agent。
// 为什么要做这个：构建 Agent 层次结构，允许父 Agent 将任务委派给子 Agent。
// 如何使用：传入父 Agent 和子 Agent 列表。返回一个新的支持 Resume 的 Agent（如果是 flowAgent）。
...
// AgentOption 是 flowAgent 的配置选项函数。
type AgentOption func(options *flowAgent)

// WithDisallowTransferToParent 禁止子 Agent 将控制权转回给父 Agent。
// 为什么要做这个：在某些流程中，我们希望强制单向流动或防止循环。
...
// WithHistoryRewriter 设置历史记录重写器。
// 为什么要做这个：自定义历史记录在 Agent 间传递时的格式。
...
// AgentWithOptions 为 Agent 包装特定于 Flow 的选项。
// 为什么要做这个：允许对标准 Agent 应用 Flow 相关的配置（如历史重写、禁止回传）。
// 如何使用：传入 Agent 和选项列表。
func AgentWithOptions(ctx context.Context, agent Agent, opts ...AgentOption) Agent {
	return toFlowAgent(ctx, agent, opts...)
}

// setSubAgents 是设置子 Agent 的内部实现。
// 它会处理 Agent 的包装、父子关系的建立以及回调函数的调用。
...
// rewriteMessage 是默认的消息重写逻辑。
// 它会将其他 Agent 的消息添加前缀说明，例如 "[agentName] said: ..."。
// 为什么要做这个：帮助当前 Agent 区分消息来源，理解上下文。
