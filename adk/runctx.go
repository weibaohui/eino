...
type agentEventWrapper struct {
	*AgentEvent
	...
	// TS 是创建此事件的时间戳（以纳秒为单位）。
	// 它主要由 laneEvents 机制用于在多 Agent 流程中对来自不同 Agent 的事件进行排序。
	TS int64
	...
	// StreamErr 存储 MessageStream 包含错误时的错误消息。
	// 当流已被消费并出错时，此字段防止多次调用 getMessageFromWrappedEvent。
	// 通常当 StreamErr 发生时，Agent 将返回错误，除非为生成此流的 Agent 配置了重试，
	// 在这种情况下，StreamErr 将是 WillRetryError 类型（表示重试挂起）。
	StreamErr error
}
...
// GetSessionValue 通过键检索 Session 值并报告其是否存在。
...
// updateRunPathOnly 创建一个更新了 RunPath 的新上下文，但不修改 Address。
// 这由顺序工作流用于累积 LLM 上下文的执行历史记录，而不会错误地链接对等 Agent 的静态地址。
...
// ClearRunCtx 清除多 Agent 的运行上下文。
// 当一个包含多 Agent 的自定义 Agent 被设置为另一个多 Agent 的子 Agent 时，这特别有用。
// 在这种情况下，不期望将外部运行上下文传递给内部多 Agent，因此此函数有助于正确隔离上下文。
